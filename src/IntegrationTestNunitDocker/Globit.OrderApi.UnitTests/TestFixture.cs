namespace Globit.OrderApi.UnitTests
{
    ...
    public class TestFixture
    {
        ...
        public static Task ExecuteDbContextAsync(Func<OrderContext, Task> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<OrderContext>()));

        public static Task ExecuteDbContextAsync(Func<OrderContext, ValueTask> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<OrderContext>()).AsTask());

        public static Task ExecuteDbContextAsync(Func<OrderContext, IMediator, Task> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<OrderContext>(), sp.GetService<IMediator>()));

        public static Task<T> ExecuteDbContextAsync<T>(Func<OrderContext, Task<T>> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<OrderContext>()));

        public static Task<T> ExecuteDbContextAsync<T>(Func<OrderContext, ValueTask<T>> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<OrderContext>()).AsTask());

        public static Task<T> ExecuteDbContextAsync<T>(Func<OrderContext, IMediator, Task<T>> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<OrderContext>(), sp.GetService<IMediator>()));
